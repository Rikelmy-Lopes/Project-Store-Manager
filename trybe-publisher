...
if [[ ! -d $SCRIPTS_BASE ]]; then  
    ...
    CURR_PATH=$PWD
    cd $SCRIPTS_BASE && git pull --quiet # caso o repositório já exista, sincroniza qualquer atualização
...
if ! git fetch origin ; then # não tem acesso ou porque não existe ou porque não tem autorização
